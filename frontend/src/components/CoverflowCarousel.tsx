...
export function CoverflowCarousel({ items, initialIndex }: CoverflowCarouselProps) {
  ...
  const getCardStyle = (index: number): CSSProperties => {
    ...
    return {
      ...
      boxShadow:
        abs === 0
          ? '0 32px 80px rgba(10, 45, 91, 0.16), 0 8px 24px rgba(0,0,0,0.12)'
          : abs === 1
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <h3 className="line-clamp-2 text-lg font-extrabold leading-tight text-gray-950 group-hover:text-[#0A2D5B]">
                  {item.title}
                </h3>
            ...
            return (
              ...
              >
                ...
                <div className="px-[22px] pb-[22px] pt-5">
                  {item.region && (
                    <div className="mb-1.5 text-[13px] font-bold uppercase tracking-[0.05em] text-[#0A2D5B]">
                      {item.region}
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="flex items-center gap-1 text-[13px] font-semibold text-[#F37021]">
          ...
          {items.map((item, index) => (
            <button
              ...
              style={{
                ...
                background: index === activeIndex ? '#F37021' : '#D1D5DB',
              }}
            />
          ))}
        </div>
...
function CoverflowImage({
  ...
  return (
    <div className="relative overflow-hidden bg-[#F37021]/10" style={{ height }}>
      {item.image ? (
        <img src={item.image} alt={item.title} className="block h-full w-full object-cover" />
      ) : (
        <div className="flex h-full w-full items-center justify-center">
          <BookOpen size={34} className="text-[#F37021]/35" />
        </div>
      )}
      ...
      {item.province && (
        <div className="absolute left-4 top-4 flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-xs font-semibold text-gray-700 backdrop-blur">
          <MapPin size={10} className="text-[#F37021]" />
          {item.province}
        </div>
      )}
      {item.tag && (
        <div className="absolute bottom-3.5 left-4 rounded-full bg-[#F37021] px-2.5 py-1 text-[11px] font-semibold tracking-wide text-white">
          {item.tag}
        </div>
      )}
    </div>

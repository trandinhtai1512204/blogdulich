/**
 * Seed script — populate cities and category records required by the sitemap.
 *
 * Idempotent: re-running will upsert without duplicating.
 *
 * Run: npx ts-node --transpile-only prisma/seed.ts
 *
 * Slug conventions follow the raw sitemap URLs (NOT normalized):
 *   /lich-trinh-du-lich-{city}    → category slug = `lich-trinh-du-lich-{city}`
 *   /kinh-nghiem-du-lich-{city}   → category slug = `kinh-nghiem-du-lich-{city}`
 *   /review-{type}                → category slug = `review-{type}`
 *
 * Display name = city name only (e.g. "Hà Nội") so breadcrumb shows
 *   "Lịch trình › Hà Nội" not "Lịch trình › Lịch Trình Hà Nội".
 */
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

// 34 provinces (post-2025 administrative reform). Edit if your set differs.
...
// Per-city flat verticals — slug pattern follows raw sitemap URL.
const VERTICALS = [
...
// Review subtypes — flat children of root `review`.
...
async function getRoot(slug: string) {
  const root = await prisma.category.findFirst({ where: { slug, parentId: null } });
  if (!root) throw new Error(`Missing system root category: ${slug}. Boot the app once to auto-create it.`);
  return root;
}

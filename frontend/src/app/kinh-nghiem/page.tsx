...
export default function ExperienceIndexPage() {
  ...
  const activeMapSlugs = new Set(categories.map((c) => getCitySlugFromExperience(c.slug)));
  ...
  return (
    <div className="relative isolate min-h-screen overflow-hidden bg-[#F6F3EE] text-[#0A2D5B]">
        ...
        <div className="relative z-10 mx-auto grid min-h-[720px] max-w-[1440px] grid-cols-1 items-stretch lg:h-[min(860px,calc(100vh-74px))] lg:grid-cols-[60%_40%]">
          ...
          <div className="relative z-10 w-full max-w-xl">
            <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-2 text-sm font-bold text-[#0A2D5B]/70">
              ...
              <span className="text-[#0A2D5B]">Kinh nghiệm du lịch</span>
            </nav>
            ...
            <div className="mb-5 inline-flex items-center gap-2 rounded-full bg-white/75 px-3 py-1 text-xs font-bold text-[#0A2D5B] shadow-[0_8px_24px_rgba(10,45,91,0.08)] backdrop-blur">
              <Backpack size={13} />
              {categories.length > 0 ? `${categories.length} cẩm nang` : 'Cẩm nang du lịch'}
            </div>
            ...
            <p className="mt-5 max-w-md text-base font-semibold leading-7 text-[#0A2D5B]/72">
              Cẩm nang thực chiến theo từng điểm đến, giúp bạn chuẩn bị chuyến đi gọn hơn.
            </p>

            {/* ── SEARCH BOX ── */}
            <div className="relative mt-8 w-full max-w-xl">
              <div
                className="flex items-center overflow-visible rounded-full bg-white/94 shadow-[0_18px_60px_rgba(10,45,91,0.14)] ring-1 ring-[#0A2D5B]/10 backdrop-blur-sm"
              >
                ...
                >
                  <div className="max-h-[380px] overflow-y-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: '#d1d5db transparent' }}>
                    {matchedCities.length > 0 && (
                      <div className="pt-3 pb-1">
                        <div className="flex items-center gap-2 px-4 pb-2">
                          ...
                          <span className="text-[11px] font-bold uppercase tracking-widest text-neutral-400">Thành phố</span>
                        </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0 text-left">
                                <p className="text-sm font-semibold text-neutral-950 leading-tight transition-colors group-hover:text-neutral-700">{cat.name}</p>
                    ...
                    {matchedPosts.length > 0 && (
                      <div className="pt-2 pb-3">
                        <div className="flex items-center gap-2 px-4 pb-2">
                          ...
                          <span className="text-[11px] font-bold uppercase tracking-widest text-neutral-400">Bài viết</span>
                        </div>
                        {matchedPosts.map((post) => (
                          <Link
                            key={post.id}
                            href={post.canonicalUrl ?? (post.category?.slug ? `/kinh-nghiem/${getCitySlugFromExperience(post.category.slug)}/${post.slug}` : `/posts/${post.slug}`)}
                            onClick={() => setFocused(false)}
                            className="group flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-neutral-50"
                          >
                            ...
                            <div className="flex-1 min-w-0 text-left">
                              <p className="truncate text-sm font-semibold text-neutral-950 leading-tight transition-colors group-hover:text-neutral-700">{post.title}</p>
                    ...
                    {hasQuery && matchedCities.length === 0 && matchedPosts.length === 0 && (
                      <div className="py-8 text-center text-sm text-neutral-500">Không tìm thấy kết quả phù hợp.</div>
                    )}
                  </div>
                </div>
              )}
          ...
          <div className="relative min-h-[640px] overflow-hidden bg-transparent lg:min-h-[720px]">
            <InteractiveVietnamMap
              ...
              hrefForSlug={(slug) => `/kinh-nghiem/${slug}`}
        ...
        {featuredPosts.length > 0 && (
          <section>
            <div className="mb-8 grid gap-7 lg:grid-cols-[0.92fr_1.08fr] lg:items-end lg:gap-20">
              <div>
                <p className="mb-2 text-xs font-extrabold uppercase tracking-[0.18em] text-[#F37021]">Cẩm nang thực chiến</p>
                <h2 className="text-4xl font-extrabold leading-[1.06] tracking-tight text-[#0A2D5B] md:text-5xl">Kinh nghiệm nổi bật</h2>
              </div>
              <p className="text-base font-medium leading-8 text-[#0A2D5B]/68 md:text-lg">
                Những bài viết giúp chuẩn bị chuyến đi gọn hơn: ăn ở đâu, tránh lỗi gì, đi thế nào cho hợp điểm đến.
              </p>
            </div>
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-5 pb-0">
                    ...
                    <div className="mt-auto space-y-2 pb-4 pt-4 text-sm font-medium text-[#1d2738]/80">
                      ...
                      <span className="flex items-center gap-3">
                        <Backpack size={15} className="shrink-0 text-[#071f3d]" />
      ...
      <FaqSection
        targetType="global"
        module="experience"
        heading="Những câu hỏi thường gặp về kinh nghiệm du lịch"
      />
    </div>
  );
}

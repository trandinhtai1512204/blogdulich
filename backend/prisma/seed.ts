...
function itineraryContent(city: string): string {
...
function experienceContent(city: string): string {
...
function reviewContent(city: string, subtypeName: string): string {
...
async function ensureSystemRoots() {
  const roots = [
    { name: 'Điểm đến hấp dẫn',   slug: 'diem-den',          type: 'destination' as const },
    { name: 'Lịch trình du lịch',  slug: 'lich-trinh-du-lich', type: 'itinerary'   as const },
    { name: 'Review',              slug: 'review',             type: 'review'      as const },
    { name: 'Kinh nghiệm du lịch', slug: 'kinh-nghiem',        type: 'experience'  as const },
  ];
...
async function getRoot(slug: string) {
  ...
  if (!root) throw new Error(`Missing root: ${slug}`);
  return root;
}
